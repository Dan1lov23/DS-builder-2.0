import './equipAdd.css';
import {FontAwesomeIcon} from '@fortawesome/react-fontawesome';
import {faTrash} from '@fortawesome/free-solid-svg-icons';

export default function EquipAdd({ userItem, removeItem, requirementsItem }) {

    let statsImgsArray = [
        {src: "https://darksouls.wiki.fextralife.com/file/Dark-Souls/icon_strength.png"},
        {src: "https://darksouls.wiki.fextralife.com/file/Dark-Souls/icon_dexterity.png"},
        {src: "https://darksouls.wiki.fextralife.com/file/Dark-Souls/icon_intelligence.png"},
        {src: "https://darksouls.wiki.fextralife.com/file/Dark-Souls/icon_faith.png"}
    ]

    return (
        <div className="equipAddMain">
            <div className="equipAddContainer">
                {userItem.map((item, index) => (
                    <div className="equipAdd" key={index}>
                        <button>
                            <img src={item.src}
                                 alt="data:image/png;base64,"/>
                        </button>
                        <button className="deleteButton" onClick={() => removeItem(index)}><FontAwesomeIcon
                            icon={faTrash}/>
                        </button>
                        <div className={"requirementsMainTitle"}>
                            <h1>requirements</h1>
                        </div>
                        {statsImgsArray.map((imgSrc, num) => (
                            <div className="requirementsMain" key={num}>
                                <img src={imgSrc.src}/><p>{String(requirementsItem).split(" ")[[num]]}</p>
                            </div>
                        ))}
                    </div>
                ))}
            </div>
        </div>
    );
}
